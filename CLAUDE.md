...
| GitHub                            | `maxnovaluciglobal-a11y/alika` rama `main` (migrado desde `walterlamadriz-ai` el 2026-08-31; la URL vieja redirige)                                                                                                                                                                                                                                                                                                                  |
| Vercel (producción)               | proyecto `alika` en el team **`maxnovaluci-global`** (cuenta empresa, plan Pro — migrado desde la org personal `walterlamadriz-8845s-projects` el 2026-08-31). Prod: `https://alika-omega.vercel.app`, Node 24.x, auto-deploy en push a `main`. ⚠️ **Sigue sin dominio propio** — pendiente abierto, y Meta lo puede pedir para la verificación de negocio del enrolamiento WhatsApp. `orgId`/`projectId` en `.vercel/project.json`. |
...
4. **`types.ts` de Supabase se parchea a MANO** cuando agregás tabla/enum nuevo. Bloque `Row/Insert/Update/Relationships` + añadir al `Enums`. ⚠️ **Esto es evitable y conviene arreglarlo**: el CLI ya está instalado (`/opt/homebrew/bin/supabase` v2.114.0) y `package.json` tiene el script `types:gen` — solo falta `supabase login` (o `SUPABASE_ACCESS_TOKEN`) para poder regenerar y hacer `git diff` en vez de auditar el drift a mano. Auditado el 04-sep: hoy NO hay drift (55/55 tablas, 669/669 columnas, 19/19 enums, 6/6 RPCs), pero los `Relationships` de 8 tablas nuevas quedaron vacíos y eso rompe cualquier embedded `select('t(col)')` futuro. Conviene además sumar `supabase` como devDependency: hoy el script depende de un binario global.
...
6. **Dinero:** `bigint` cents. `formatMoney(cents, currency)` respeta ISO zero-decimal (CLP/PYG/COP/JPY/etc). Nunca dividir por 100 asumiendo. `currency` es **obligatorio** en `formatMoney`/`toCents`/`fromCents`: sin default, el compilador encuentra los sitios que no tienen la moneda a mano. Todo input de dinero va con `MoneyInput`, que habla cents y muestra la unidad visible con el `step` correcto.
...
12. **Nunca `||` para caer a un default cuando 0 es un valor legítimo** — usar `??`. Ya causó un bug real: `patient_cents` de 0 (convenio que cubre el 100 %) caía al precio de lista y le cobraba al paciente lo que el seguro ya había pagado. Ver `fetchPatientBalances`.
13. **Nada de caracteres invisibles en el fuente.** Los rangos de diacríticos van escapados (`\u0300-\u036f`), nunca con los caracteres literales. `scripts/check-invisibles.mjs` corre en lint-staged y lo rechaza — se agregó después de que pasara tres veces el mismo día (un byte NUL y dos regex).
14. **La moneda nunca viaja desde el cliente.** Ninguna server function acepta `currency` en su schema: el trigger `moneda_desde_la_clinica` (BEFORE INSERT en `expenses`, `lab_orders`, `payments`, `procedures`, `quotes`, `treatment_plans`) la fija desde `clinics.currency` y las columnas ya no tienen `DEFAULT 'CLP'` — si el trigger falta, el INSERT revienta contra el NOT NULL en vez de escribir Chile en silencio. Es la misma lección que `created_by`: un DEFAULT solo aplica si el cliente omite la columna, un trigger que sobreescribe siempre hace imposible el error.
15. **Todo gate de permiso de la UI necesita su par en el servidor.** El JWT vive en `localStorage`, así que cualquier miembro puede llamar un server fn directo. Para dinero, el helper es `requireFinanceView` (`finance-reports.functions.ts`). RLS sola no alcanza: `treatment_items_select_members` y `payments_select_members` dejan leer a roles sin `finance:view`.
...
- **Webhook:** `src/routes/api.whatsapp-webhook.ts` — status callbacks (delivered/read/failed) + mensajes entrantes: guarda el mensaje en `messages` con `direction='inbound'` y corta `wa_opt_in` si el texto es BAJA/STOP/CANCELAR/UNSUBSCRIBE. **NO confirma citas** — el webhook no toca `appointments` en ninguna línea. El auto-confirm por "SI" del paciente existió y se removió a propósito en `505eb7d` (01-sep-2026): `confirmada` significa "el odontólogo aceptó la cita", no "el paciente avisa que viene", y dejar que el paciente lo escribiera le daba permiso de escritura sobre la agenda del profesional (ver `can_confirm_appointment` + RLS de la migración 20260901130000). Si alguna vez se quiere confirmación de asistencia por WhatsApp, va como **eje nuevo y separado** (ej. `patient_confirmed_at`), nunca reusando `confirmed_at`/`confirmed_by`.
...
- **Auditoria de notas clinicas (`clinical_note_audit`): la escribe la base, no la app.** Hasta el 07-sep la politica `note_audit_insert_members` dejaba que **cualquier miembro** insertara filas con `action` y `detail` arbitrarios — se podia fabricar un "firmo" o un "aprobo la revision" que nunca paso. Migracion `20260907200000`: la politica de INSERT **se elimino** y quedaron dos caminos, segun si el hecho es observable o no. (a) Lo que la base conoce lo escriben **5 triggers** (`note_versions_auditar`, `notes_auditar_estado`, `note_reviews_auditar`, `note_entities_auditar_confirm`, `note_entities_auditar_delete`) via `anotar_auditoria_de_nota()`. (b) Lo que la base **no puede ver** —un choque de la cola offline, una llamada al modelo de IA— pasa por `registrar_evento_de_nota()`, que solo acepta `conflict`/`ai_*` y **estampa el actor ella misma**. Un `CHECK` cierra el vocabulario. Lo peor que puede inventar hoy un usuario es "use IA"; nada sobre la integridad del registro clinico. Para que "revertir" fuera un hecho de la base y no texto libre, `clinical_note_versions` gano `origin`/`origin_version`. **No agregar `.from("clinical_note_audit").insert(...)` en ningun lado: falla por RLS.**
- **Pagina de efectividad (F5, `/efectividad` + `efectividad.ts`): mide si la automatizacion sirve, sobre 90 dias de datos reales.** Ausencias, ausencia con y sin recordatorio, ausencia segun haya avisado el paciente, cuantos mensajes entrantes se resuelven solos, mediana de respuesta del equipo y pedidos de hora por WhatsApp que terminaron agendados. ⭐⭐ **`proporcion()` devuelve `porcentaje: null` por debajo de `MUESTRA_MINIMA` (20) y la pantalla muestra el conteo crudo**: con pocos casos un porcentaje parece una conclusion y no lo es (1 de 2 no es "50%"). ⚠️ **Las comparaciones son OBSERVACIONALES, no experimentos** — la clinica elige a quien le manda recordatorio, y eso puede correlacionar con quien iba a faltar igual; la pantalla lo dice en voz alta y no hay que convertirlo en promesa de marketing. ⚠️ El complemento de "mensajes que se resuelven solos" **no es todo automatizable** (un "gracias", una consulta de precio, una foto): es el techo de lo que un LLM podria ganar, no lo que se esta perdiendo. La mediana de respuesta **excluye los hilos sin responder**: no son una respuesta lenta, son otra cosa, y se muestran aparte. Los evals del modelo entran aca cuando existan las API keys; lo que ya se mide es el eval de las REGLAS de F4. Cero migraciones.
- **Lectura de intencion de agenda (F4, `intencion-de-agenda.ts`): reglas primero, modelo despues.** Cuando un paciente escribe por WhatsApp, el webhook intenta leer si pide, mueve o cancela una hora con **reglas deterministas** (verbos + palabra de cita + fecha + franja). Si lo lee, el aviso al equipo dice que pidio y para cuando, y **si la intencion es `agendar` y hay fecha** se crea una fila en `appointment_requests` con `source='whatsapp'` — el mismo circuito que ya usa el portal. Mover o cancelar NO crean fila: tocan una cita concreta que hay que elegir, y eso es decision de la clinica. ⭐ **El `null` de `interpretarMensajeDeAgenda` es el seam del LLM**, no un error: hoy cae en el aviso generico de siempre (no se pierde nada) y el dia que existan `GEMINI_API_KEY`/`OPENAI_API_KEY` ese 20% ambiguo pasa al modelo sin tocar lo que ya funciona. El orden en el webhook es **baja → confirmacion → agenda → generico**, y esta fijado en `tests/agenda-precedencia-webhook.test.ts`: "CANCELAR" pelado sigue siendo una BAJA (Ley 21.719), mientras que "quiero cancelar mi hora" se lee como cancelacion de cita. **La fecha se calcula con `hoyEnLaClinica(clinics.timezone)`, nunca con el reloj del servidor** — el handler corre en UTC y "mañana" escrito a las 22:00 en Santiago se correria un dia. Cero migraciones: `source='whatsapp'` ya estaba en el CHECK.
- **Resumen diario (`/api/daily-digest`, cron horario en `vercel.json`):** el único cron que toca mensajería, y **no le escribe a ningún paciente** — cuenta lo pendiente y avisa al equipo dentro de la app (`kind='daily_digest'`). Es la forma de honrar la decisión de arriba sin dejar el trabajo dependiendo de que alguien se acuerde de entrar. Cinco frenos: hora local de la clínica (corre cada hora, actúa solo a las 8 de `clinics.timezone`), una sola vez por día (ventana de 20 h sobre `notifications`, no medianoche local: evitar aritmética de calendario es evitar el bug de DST), silencio total si no hay nada pendiente, la clínica demo excluida, y tope de clínicas por corrida. Lógica pura en `src/lib/messaging/digest-diario.ts` con 14 tests que **no tocan Postgres** — el CI corre contra la base de producción, así que un test que dependa de una columna nueva deja el CI rojo hasta que alguien aplique la migración a mano. **Cuenta los recordatorios con un criterio más grueso que `/recordatorios` a propósito**: la pantalla deduplica por canal, y como el canal email no está habilitado (falta `RESEND_API_KEY`), toda cita quedaría "pendiente" para siempre y el número nunca bajaría.
...
- **⚠️ Cambios en `vercel.json` (redirects, headers estáticos) NO se prueban en local** — el dev server (`npm run dev`) no aplica esos headers, solo el deploy de Vercel. Verificar SIEMPRE en un preview deploy real. El CSP con `script-src 'self'` (sin `'unsafe-inline'`) tumbó producción entera el 2026-08-15 porque bloqueaba los scripts inline de hidratación de TanStack Start — ver memoria `alika_csp_incident`.
- **CSP con nonce real, en producción desde el 06-sep-2026** (`src/router.tsx`) — a diferencia de los demás headers, el `Content-Security-Policy` YA NO vive en `vercel.json`: se genera por request (`crypto.getRandomValues`, 128 bits) dentro de `getRouter()`, vía `createIsomorphicFn().server(...)` (necesario porque el plugin de Vite de TanStack Start bloquea en build cualquier import de `@tanstack/react-start/server` alcanzable desde el bundle de cliente, aunque esté detrás de un guard en runtime), y se pasa a `createRouter({ ssr: { nonce } })` — el propio router lo threadea a cada script/meta que inyecta (`Scripts`/`HeadContent`/`Asset`), incluido el manifest de rutas con timestamp por request que rompía el intento anterior. Por eso **sí se puede probar en local** (`npm run dev` + `curl -sD - http://localhost:PUERTO/auth`): el header ya no depende de Vercel. `style-src` sigue con `'unsafe-inline'` a propósito — Radix UI inyecta `style=""` en runtime (atributo, no elemento) y CSP nonce nunca cubre atributos.
...
- **Naming**: alineado con la marca el 2026-08-31 — carpeta local `05 - Alika/` y repo `maxnovaluciglobal-a11y/alika` (la URL vieja `aurora-dental-os` redirige con 301). La restricción de "nunca renombrar por la sync de Lovable" **ya no aplica**: Alika se desacopló de Lovable, ver `docs/DESACOPLE_LOVABLE.md`.
