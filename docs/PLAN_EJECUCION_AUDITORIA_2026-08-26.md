...
| 2   | Pegar `B2_ACCOUNT_ID` / `B2_APPLICATION_KEY` (y el nombre de bucket si difiere) como GitHub Secrets del repo `aurora-dental-os`                                                                                                                              | Oleada 0 (seguridad-1)                                                                                                                     |
...
- **Acción de Walter (no delegable):** cargar `B2_ACCOUNT_ID` y `B2_APPLICATION_KEY` como GitHub Secrets en `walterlamadriz-ai/aurora-dental-os` → Settings → Secrets and variables → Actions.
- **Mi verificación después:** `gh run list --workflow=backup.yml -R walterlamadriz-ai/aurora-dental-os --limit 3` y `gh run view --log <run-id>` sobre la corrida disparada manualmente (`gh workflow run backup.yml`). Confirmar en el log que el paso "subir a B2" termina en éxito, no en `account not found`. **No dar esto por resuelto solo porque Walter dice que pegó los secrets** — correr y leer el log.
