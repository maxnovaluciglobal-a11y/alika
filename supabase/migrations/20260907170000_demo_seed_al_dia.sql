-- ════════════════════════════════════════════════════════════════════════
-- La demo pública se pone al día con el producto (07-sep-2026)
-- ════════════════════════════════════════════════════════════════════════
--
-- Dos problemas, uno más grave que el otro.
--
-- 1. **El seed enriquecido del 06-sep nunca se aplicó en producción.** Define
--    10 pacientes; la clínica demo real tenía 4, y cero mensajes. O sea que
--    /demo llevaba días mostrando una versión empobrecida del producto a
--    cualquier prospecto que entrara. Verificado contra producción antes de
--    escribir esto, no deducido del archivo en disco.
--
-- 2. **Se quedó atrás de todo lo construido después.** Cubría 22 de 57
--    tablas: faltaban boxes, anamnesis, bodegas, movimientos de inventario,
--    receta de insumos por prestación, conteos físicos, planes de tratamiento
--    con fases y copago, gastos, comisiones, retención de medios de pago,
--    captación por WhatsApp, solicitudes de hora y el aviso del paciente.
--
-- Esta migración reemplaza el cuerpo entero de `reset_demo_clinic()` con la
-- versión al día. Mismos IDs de clínica/dueño/sucursales/pacientes que ya
-- usaba: nada que los referencie se rompe.
--
-- Tres cosas que se aprendieron ejecutándola, no leyéndola:
--
--  · **El stock no se escribe a mano.** Los ítems arrancan en 0 y se cargan
--    con `inventory_movements` reales. Sembrar `current_stock` directo —que
--    es lo que hacía la versión anterior— deja `inventory_stock` vacío: el
--    total del ítem sin respaldo en ninguna bodega, justo el descuadre que se
--    arregló el 07-sep.
--
--  · **El plan de tratamiento no se inserta.** `quote_accept_creates_plan` ya
--    lo crea al aceptar el presupuesto, y meter uno a mano choca contra
--    `treatment_plans_quote_unico`. Acá solo se lo enriquece con lo que el
--    trigger no puede saber: fases, profesional, avance y reparto
--    convenio/paciente.
--
--  · **`created_at` va explícito en los mensajes.** Su default es `now()`, así
--    que fijar solo `sent_at` dejaba los recordatorios "viejos" como el
...
--
-- Verificado contra la réplica local antes de aplicar: el invariante
-- `inventory_items.current_stock == SUM(inventory_stock.current_stock)` se
-- cumple en los 8 ítems, `coverage_cents + patient_cents = price_cents` en
-- los 4 ítems del plan, y no quedan ítems duplicados.
