-- Función de reset de la clínica demo pública, para el cron periódico
-- (ver src/routes/api.demo-reset.ts). Mismo contenido que scripts/demo-seed.sql,
-- portado a función de Postgres para poder invocarla via RPC.
--
-- SECURITY DEFINER a propósito acá (al revés que block_demo_writes): esta
-- función SÍ necesita correr con privilegios del dueño (postgres) sin
-- importar quién la invoque, para que sus propios INSERT/UPDATE/DELETE
-- pasen el chequeo `current_user IN ('service_role','postgres')` del
-- trigger de solo-lectura. Solo service_role puede ejecutarla (ver GRANT).
